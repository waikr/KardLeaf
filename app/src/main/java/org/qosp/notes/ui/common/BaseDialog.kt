...
abstract class BaseDialog<V : ViewBinding> : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val themedContext = ContextThemeWrapper(requireContext(), R.style.Theme_KardLeaf_QuillpadEditor)
        return MaterialAlertDialogBuilder(themedContext)
            .apply {
                _binding = createBinding(LayoutInflater.from(themedContext))
    ...
    companion object {
        inline fun build(context: Context, block: MaterialAlertDialogBuilder.() -> Unit): AlertDialog {
            val themedContext = ContextThemeWrapper(context, R.style.Theme_KardLeaf_QuillpadEditor)
