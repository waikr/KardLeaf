...
class EditorFastScrollEdgeView(context: Context) : View(context) {
    ...
    private val trackTapMaxDurationMs = ViewConfiguration.getLongPressTimeout().toLong()
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                val shouldHandleFastScroll = canScroll
                if (!shouldHandleFastScroll && !isSidePanelDragging) {
                    return false
                }
                downX = event.x
                downY = event.y
                ...
                if (canScroll && isTouchOnThumb(event.y)) {
                    ...
                    pendingFastScroll = shouldHandleFastScroll
                }
                return true
            }
            MotionEvent.ACTION_MOVE -> {
                ...
                if (pendingFastScroll && absDy > touchSlop && absDy > absDx) {
                    startFastScroll(event.y, centerThumbOnTouch = true, haptic = false)
                    return true
                }
                if (!canDragSidePanel && pendingFastScroll && absDx > touchSlop) {
                    pendingFastScroll = false
                }
                return true
            }
            MotionEvent.ACTION_UP -> {
                when {
                    ...
                    pendingFastScroll && canScroll && event.eventTime - event.downTime < trackTapMaxDurationMs -> {
                        startFastScroll(event.y, centerThumbOnTouch = true, haptic = false)
                        performClick()
                    }
